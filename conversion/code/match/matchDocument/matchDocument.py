...
class MatchDocument():
    ...
    '''
    This class is used to perform the heading extraction and validation for a particular ePI document of a specific document type.
    As a result of this process a collection of qrd headings found in the ePI document is generated.
    Each entry in the collection is nothing but the specific row in the html dataframe (Generated by reading the partitionedJSONs as pandas dataframe)
    '''
    def __init__(self,
                    logger: MatchLogger,
                    controlBasePath,
        ...
        '''
        This init function is used to initialize the object of the this class using above parameters.
        Steps completed as part of initialization :- 
        - Convert the partitionedJSON in to pandas dataframe named dfHTML
        - Create a mapping dict for mapping document type value from qrd to document number
        - Using the input document number the document type is extracted from the above dict
        - Create the file handler for writing missed heading in each section to a file.
        - Generate the QRD template dataframe as dfModelwRulesF
        - Read the match rulebook
    ...
    def updatedParentId(self, currentHeadingRow, previousHeadingRowFound, collectionFoundHeadings, subSectionIndex, dfQrd):
        ...
        '''
        This function is used to update the parent id in the final collection if the parent heading is missing in the collection.
        It replaces the actual missed parent id with the previous heading's parent id such that the child parent flow is not broken.
        '''
    ...
    def storeResults(self, collection_, qrd_str_row, str_, indexDF, subSectionIndex, doc_parent_id):
        '''
        This function is used to add the new heading entries in to the result collection.
        Following are the columns added for each heading found.
        '''
